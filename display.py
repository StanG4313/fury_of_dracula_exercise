class Display:
    ...
    def phrase(self, phrase):
        phrases = {
            ...
            "no_result": {"EN": "No result", "RU": "Нет результатов игры"},
            "dracula_win": {"EN": "Dracula wins!", "RU": "Победа Дракулы!"},
            "hunters_win": {"EN": "Hunters win!", "RU": "Победа охотников!"},
            "the_end": {"EN": "Game ended", "RU": "Игра окончена"},
            "default_config": {
            ...
            "play_day": {"EN": "Current phase: Day", "RU": "Наступает день"},
            "play_sunset": {"EN": "Current phase: Sunset", "RU": "Наступает закат"},
            "play_night": {"EN": "Current phase: Night", "RU": "Наступает ночь"},
            "play_sunrise": {"EN": "Current phase: Sunrise", "RU": "Наступает рассвет"},
            "wanna_load": {
    ...
    def engine_startup_with_timestamp(self, phrase, timestamp):
        phrases = {
            "start": {"EN": "Game engine started at", "RU": "Движок игры запущен в"},
            "end": {"EN": "Game engine ready at", "RU": "Движок игры готов в"},
        }
    ...
    def effect_error(self, effect):
        phrase = {
            "EN": "Effect "
            + effect
            + " from preset active_effects field is not in available effects in config.",
            "RU": "Эффект "
            + effect
    ...
    def current_hunters_position(self, players, hunters_indexes, final=False):
        ...
        for index in hunters_indexes:
            if (
                self.language == "RU"
                and players[index]["dynamic"]["location"] == "undefined"
            ):
                players[index]["dynamic"]["location"] = "не указано"
            print(
                players[index][name[self.language]],
                ": ",
                players[index]["dynamic"]["location"],
                sep="",
            )
        print()
        ...
        for location in locations:
            ...
            if location["type"] == "city":
                ...
                print(
                    location["id"],
                    ") ",
                    location[key.get(self.language, "EN")],
                    filler,
                    size,
                    sep="",
                )
                ...
                print(
                    location["id"], ") ", location[key.get(self.language, "EN")], sep=""
                )
    ...
    def available_move_locations(self, locations_available):
        ...
        phrase = {
            "RU": [
                "Доступные локации:",
                'Укажите новую локацию, или введите "CANCEL" чтобы вернуться в предыдущее меню',
            ],
            "EN": [
                "Available locations:",
                'Specify a new location, or enter "CANCEL" to return to the previous menu',
            ],
        }
    ...
    def ask_player_to_choose_start_location(self, hunter):
        name = {"EN": "name_en", "RU": "name_ru"}
        phrase = {
            "EN": ", please, specify spawn location NUMBER from available:",
    ...
    def ask_hunter_to_confirm_readiness(self, hunter):
        name = {"EN": "name_en", "RU": "name_ru"}
        phrase = {
            "EN": "please, confirm readiness by typing Y below:",
    ...
    def loadsave_report(self, file, load=False):
        process = (
            {"EN": "loaded from file", "RU": "загружено из файла"}
            if load
            else {"EN": "saved to file", "RU": "сохранено в файл"}
        )
    ...
    def current_player(self, player):
        name = {"EN": "name_en", "RU": "name_ru"}
        phrase = {"EN": "Current player:", "RU": "Текущий игрок:"}
        print()
        print(
            phrase[self.language],
            player[name[self.language]],
        )

    def actions_available(self, actions_dict):

        phrase = {"EN": "Choose your action:", "RU": "Выберите действие"}
        action_names = {
            "move_by_road": {"EN": "Move using roads", "RU": "Передвинуться по дороге"},
            "move_by_railway": {
                "EN": "Move using railroads",
                "RU": "Передвинуться по железной дороге",
            },
    ...
    def player_moved(self, player, move_from, move_to, way_of_transportation):
        how = {
            "RU": {"road": "(по дороге)", "sea": "(по морю)"},
            "EN": {"road": "(using roads)", "sea": "(using seas)"},
        }
        ...
        if self.language == "RU":
            print(
                ...
                player["name_ru"],
                "перемещён из",
                move_from.get("title_ru", "госпиталя"),
                "в",
                move_to["title_ru"],
                how["RU"][way_of_transportation],
                "успешно",
            )
        else:
            print(
                ...
                player["name_en"],
                "moved from",
                move_from.get("title_en", "hospital"),
                "to",
                move_to["title_en"],
                how["EN"][way_of_transportation],
                "successfully",
            )
